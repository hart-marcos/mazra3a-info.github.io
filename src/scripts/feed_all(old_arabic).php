...
if($uid != null && $sr != null && $type_ != null){
...
$urls = file_get_contents("../links/". $type. ".json");
...
foreach ($array as $value)
{
    ...
    try
    {
        ...
        if (strpos($response, 'يساعد') || strpos($response, 'بذور الجديدة') || strpos($response, 'وهي تنتظرك'))
        {
            $nbr++;
            $cont++;
        }
...
if ($no_valid > 11 || (count($array) < 11 && ($type == 't_bingo' || $type == 't_get_farmclub_final_reward' ) ) || $array = [] || count($array) < 6)
{
    ...
    $urls = get_web_page("https://ff-tools.000webhostapp.com/update_urls.php?type=". $type);
